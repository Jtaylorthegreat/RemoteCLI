...
$BUILDASSIGNQUERY="SELECT Hostname FROM clientassignments WHERE GroupName is NULL or GroupName!='$GVALUE'";
$query_nongroup_agents = mysqli_query($connection, $BUILDASSIGNQUERY);
...
echo  "
        ...
        echo "<div class='tab'> <input name='css-tabs' id='tab-3' checked='checked' class='tab-switch' type='radio'> <label for='tab-3' class='tab-label'><a href='groups.php'>Agent Groups</a></label>
          <div class='tab-content'>";
            ...
            echo "<b> Select Agents to assign to group: </b><br>";
            echo "<hr>";

            while($row = mysqli_fetch_array($query_nongroup_agents))
            {
                $agent_option = $row['Hostname'];
                echo "<form method='post'><input type='checkbox' name='checkedagents[]' value = '$agent_option'> $agent_option <br>";
            }
            echo "<br><input type='submit' name = 'assignagents' value='Assign Agents'></form>";
            if(isset($_POST['assignagents'])){
                if(empty($_POST['checkedagents'])){
                    echo "Please select agents to assign to group<br>";
                }
                else {
                    foreach ($_POST['checkedagents'] as $pickedagents) {
                        $build_assign_agent_group = "UPDATE clientassignments SET GroupName='$GVALUE' WHERE Hostname='$pickedagents'";
                        if (($assign_agents_to_group = mysqli_query($connection, $build_assign_agent_group)) === TRUE ){
                            echo "$pickedagents assigned to group<br>";
                        } 
                        else {
                            echo "Error assigning $pickedagents to group <br>";
                        }
                    }  
                }
            }
