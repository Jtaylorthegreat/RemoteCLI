...
$query_agentgroup_info = mysqli_query($connection, "SELECT GroupName from groupassignments");
...
echo  "
        ...
        echo "<div class='tab'> <input name='css-tabs' id='tab-2' checked='checked' class='tab-switch' type='radio'> <label for='tab-2' class='tab-label'><a href='agents.php'>Agents</a></label><div class='tab-content'>";
            ...
            echo "Subscribe $AGENTID to Group: <br>";
            while($row = mysqli_fetch_array($query_agentgroup_info))
            {

                $group_option = $row['GroupName'];
                echo "<form method='post'><input type='radio' name='selectedgroup' value = '$group_option'> $group_option<br>";

            }
            echo "<input type ='submit' name='submitgroup' value='subcribe'><br>";
            echo "</form>";
            if(isset($_POST['submitgroup'])){
                if(empty($_POST['selectedgroup'])){
                    echo "Please select group<br>";
                }
                else {
                    $subtogroup = $_POST['selectedgroup'];
                    $build_assign_group_update = "UPDATE clientassignments SET GroupName='$subtogroup' WHERE Hostname='$AGENTID'";
                    if (($subscribe_agent_to_group = mysqli_query($connection, $build_assign_group_update)) === TRUE ){ 
                        echo " $AGENTID subscribed to $subtogroup<br>";
                    }
                    else {
                        echo "Error subscribing $AGENTID to $subtogroup<br>";
                    }
                }
            }
